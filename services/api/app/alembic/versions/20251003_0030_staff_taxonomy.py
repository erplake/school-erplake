...
def upgrade():
    # 1. Add photo_url column if not exists (Alembic doesn't have IF NOT EXISTS, so guard)
    conn = op.get_bind()
    ...
    # 3. Create staff_departments table
    ...
    # 5. Create trigger function + triggers for updated_at if absent
...
def downgrade():
    ...
    # Removing photo_url column (only if exists)
    conn = op.get_bind()
    existing_cols = [r[0] for r in conn.execute(sa.text("""
        SELECT column_name FROM information_schema.columns
        WHERE table_name='staff'
